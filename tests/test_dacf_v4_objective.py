...
from dacf_v4_objective import compute_dacf_v4_loss  # noqa: E402
from dacf_v4_relation import DACFV4Relation  # noqa: E402


class DACFV4ObjectiveTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_finite_loss_and_gradients(self) -> None:
        ...
        self.assertGreater(float(self.model.query_projection[-1].weight.grad.abs().sum()), 0.0)
    ...
    def test_rejects_positive_activity_without_silence(self) -> None:
        view1, view2 = self._outputs()
        labels = torch.tensor([[1, 1, 0, 0, 0, 0]], dtype=torch.float32)
        activity = torch.zeros(1, 6, 16)
        activity[:, :2] = 1.0
        with self.assertRaisesRegex(ValueError, "active and inactive"):
            compute_dacf_v4_loss(
                view1,
                view2,
                presence_labels=labels,
