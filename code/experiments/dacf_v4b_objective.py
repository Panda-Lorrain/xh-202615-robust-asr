"""All-pairs mechanism objective for DACF-v4b.

This is the v4 objective (``dacf_v4_objective``) with one robustness fix:
``_balanced_positive_activity`` no longer raises when a present pair has only
active or only inactive frames.  The v4b cache reuses AISHELL-1 read speech,
and a small number of present speakers are voiced for every frame of their
destination mixture.  For such a pair the activity head should simply learn an
all-active (or all-inactive) timeline; the balanced 0.5/0.5 average reduces to
the single available class.

The change is behaviourally identical to v4 on every balanced pair (the only
kind the v3/v4 caches contained), so it does not alter any frozen v4 result.
It exists only so v4b training does not abort on a degenerate edge case.  The
presence term -- the actual membership signal -- is untouched.
"""
...
from dacf_v4_objective import DACFV4Loss, DACFV4LossWeights
...
def _balanced_positive_activity(logits: Tensor, targets: Tensor) -> Tensor:
    """Balance active/inactive frames per present pair, robust to a single class.

    A present pair that is active on every frame (or inactive on every frame)
    contributes only its available class instead of raising.  Balanced pairs
    receive the same 0.5/0.5 average as the v4 objective.
    """

    raw = F.binary_cross_entropy_with_logits(logits, targets, reduction="none")
    active = targets > 0.5
    inactive = ~active
    active_total = active.sum(dim=1)
    inactive_total = inactive.sum(dim=1)
    if bool((active_total < 1).all().item()) and bool((inactive_total < 1).all().item()):
        raise ValueError("positive activity targets have no frames at all")
    active_loss = (raw * active).sum(dim=1) / active_total.clamp(min=1)
    inactive_loss = (raw * inactive).sum(dim=1) / inactive_total.clamp(min=1)
    has_active = active_total >= 1
    has_inactive = inactive_total >= 1
    both = has_active & has_inactive
    per_pair = torch.where(
        both,
        0.5 * active_loss + 0.5 * inactive_loss,
        torch.where(has_active, active_loss, inactive_loss),
    )
...
def compute_dacf_v4b_loss(
    ...
    *,
    ...
    """Compute the fixed two-view all-pairs relation objective (v4b robust)."""
    ...
    positive_activity = 0.5 * (
        _balanced_positive_activity(view1.activity_logits[present], targets[present])
        + _balanced_positive_activity(view2.activity_logits[present], targets[present])
    )
    foreign_activity = 0.5 * (
        F.binary_cross_entropy_with_logits(view1.activity_logits[absent], targets[absent])
        + F.binary_cross_entropy_with_logits(view2.activity_logits[absent], targets[absent])
    )
