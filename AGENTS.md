...
> ⚠️⚠️ **2026-08-05 反瓶颈审计（覆盖性框架，优先于下方所有"封顶/到顶/别再投/唯一治本"措辞）**：详见 `docs/反瓶颈审计与后续Agent作战令_2026-08-05.md` + `docs/反瓶颈审计补充_speaker-aware_2026-08-05.md`。**核心观念：区分 `implementation-NO-GO`（某套代码/参数/数据没过门槛）与 `direction-NO-GO`（整个方向无空间）**——下方较早段落的"封顶/别再投/到顶/exhausted/唯一治本"是**具体实现/规则/合成数据**层面的 NO-GO，**不是方向封顶**。已封住的是"在 A 集不训练 + 合成失配数据训 + 接当前 Qwen/声纹拒识栈"的低成本路径；**尚未封顶（direction-unresolved）**：①同环境/真实录音 TSE 全量验证 ②真实录音域 TSE（REAL-PS4/AISHELL-4/自录家居）③目标条件化 ASR 联合训练（target embedding/activity + ASR loss 真实域）④非内容拒识校准（Qwen decoder 置信度/token entropy/音频质量/双路一致性，非 A 数据标定）⑤分场景路由提交 A/B（+0.99 待 L20 batch1 端到端）。**裁决纪律四标签**：`implementation-NO-GO` / `conditional-GO` / `direction-unresolved` / `integrate-GO`；小收益实验必须同进程配对 + bootstrap CI + pos CER/neg RR/RTF 同看。**下方 07-29~08-05 较早段落为历史记录，其条件性 NO-GO 解释以审计文件为准，勿把"某实现 NO-GO"误读成"整个方向无空间"**。**08-05 提交线真值**：累计池 CER 0.6201（腿 15.20）/ RR 94.73%（腿 37.89）/ Overall 0.6636（~第9）；w1=w2=0.5（榜单反推）。详见 CLAUDE.md 顶部 08-05 段 + `docs/全量提交评测_2026-08-05.md`。
...
- **`code/` 代码区**：⭐`submit_infer.py`（标准化推理入口，4 阶段 subprocess）/ `enroll_infer.py`（wespeaker 声纹+diar+DiCoW 转 target）/ `se_denoise.py`+`noise_classify.py`（DeepFilterNet3 条件化）/ `llm_reject.py`（Qwen2.5-3B）/ `eval_metrics.py`+`eval_full_test.py`（评测，⚠️ `eval_metrics.py` 无 CLI，复用需 `import cer()` 或用 `eval_full_test.py` 包装）/ `simulate_pipeline.py`（仿真）/ DiCoW-inference、TS-ASR-Whisper（开源仓库，gitignore 不入库）
- **`test_wav/` 测试音频（开发调试用）**
...
2. ⚠️ **CER 破局战略决策**（组合主线 CER 1.4 是架构极限）：①保底交付(langfix+sim_thr=0.4 RR 99%+答辩归因，确定) ②端到端联合 X(反 cascaded，AGENTS.md 上限候选，大工程) ③babble 专用源分离(SepFormer 提 target mel，高成本) —— **2026-07-06 Phase 1 已开拓第④路线：vanilla Whisper + target timeline extraction（zero-training，CER 1.25→0.664 减半），无需大工程即可斩获大部分 CER 收益**
