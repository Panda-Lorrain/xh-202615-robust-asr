...
>
> **当前恢复点**：实现 DACF-v4b 数据协议，不再给 v3/v4a 续训、换 seed 或扫参。train 固定 48 speaker x 6 round=`96` group，每人严格 `2A+2B+2C`；dev 12 位未见 speaker 构造 24 group；每次 present 用 recognition/view1/view2 三个不同源 WAV、absent 用两个不同源 WAV，全局 path/SHA 使用次数=1；A/B 混音增强必须交换对称。all-pairs label 必须按 destination mixture membership 重算。当前仍未得到 CER 改善数字，提交线保持累计池 CER `0.6201`、RR `94.73%`、Overall `0.6636`。L20 暂不可租不阻塞机制门。完整文档：`docs/DACF双锚点反事实前端研究计划_2026-08-06.md`。
>
> **DACF-v3 正式结果**：Qwen-exact log-mel 与 ECST G1 均通过后，使用 AISHELL official train/dev/test=`96/12/6` 组、固定 1,920 update、无 early stop/扫参、threshold=`0.5`、2000 次 group bootstrap。checkpoint 在 dev 前冻结，dev 只观察一次，前后 SHA 均为 `3d03e238...8f41`，official final 未打开。两个 enrollment view 的 presence AUC=`0.462/0.417`、activity AUC=`0.610/0.608`、recall=`0.583/0.583`、RR=`0.417/0.333`、query-response=`-0.032/-0.065`、permutation AUC drop=`-0.028/-0.128`，所有固定门失败。精确裁决是 **frozen CAM++ + FiLM ECST v0.2=`implementation-NO-GO`**；没有运行 Qwen、CER、official neg RR 或 RTF。
>
> **v3 根因与 v4 路线**：冻结 checkpoint 的 train-only 反查中 presence AUC=`0.935/0.951`、query-response=`0.444/0.453`，而 dev 跌到随机以下；train activity AUC 仍约 `0.635`、permutation drop 仅约 `0.064`。v0.2 每位 train speaker 只出现于一个组和一个角色，模型走了 speaker→role/label 记忆捷径，没有学会 mixture-query identity。v4 同时堵两处：①同一 speaker 跨环境、源句和 present/absent 角色轮换；②逐帧 identity key 与 query 的显式 cosine/relation bottleneck，外加跨 batch foreign enrollment negatives、query-only/mixture-only ablation 门。方向总标签仍为 `direction-unresolved`。
>
> **v4a all-pairs 正式结果**：每个 8-mixture batch 只取 16 个 A/B query，每个 query 同 update 恰好 1 positive+7 foreign，C-only 不训练；30 epoch、360 update、foreign pair 覆盖 `89.89%`，预注册 `dfb0d6ac...4f273`。dev view1/view2 presence AUC=`0.682/0.664`、activity=`0.629/0.626`、RR=`0.443/0.462`、top2=`0/0`、query-response=`-0.033/-0.036`，裁决 **learnable query projection all-pairs=`implementation-NO-GO`**。train-only 发现 raw CAM++ 同/异余弦=`0.916/0.436`，projection 后=`0.988/0.921`，身份空间被压扁。
>
> **v4a anchor A/B**：删除 query projection/MLP，raw CAM++ 512d 只归一化，其他数据/loss/seed/步数/gate 完全相同，预注册 `e1318a95...7e03f`。train AUC 提到 `0.784/0.785`，但 dev AUC=`0.659/0.650`、activity=`0.629/0.631`、RR=`0.326/0.341`、top2=`0.167/0.167`、query-response=`-0.053/-0.045`，仍 `implementation-NO-GO`。这说明 query collapse 不是唯一根因；每位 speaker 只有一个正 mixture，mixture key 没学成跨 utterance/environment 身份。两个 run checkpoint 均在 dev 前冻结且观察前后 SHA 不变，final 均未打开。
>
> **测试与数据状态**：v3+v4 relation/anchor/objective/all-pairs trainer 共 `69 passed, 1 skipped`；唯一 skip 是 Windows 当前账户无 symlink 创建权限。正式 cache 有 108 组/324 query 行，Dataset-A=0，train/dev overlap 全为空，final deferred；cache SHA=`c92e0d...c045`，manifest SHA=`9c1f75...c87d`。v4b 容量已现场核对：AISHELL train/dev/test speaker=`340/40/20`、WAV=`120418/14331/7176`、每 speaker 最少=`312/335/335`，足够全局不复用源 WAV。该轮没有测官方 CER、neg RR、clean degradation 或 batch1 RTF，也没有真实家居 hard negative，不得冒充集成收益。
